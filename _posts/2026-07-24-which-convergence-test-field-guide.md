...
**Step 0: the divergence check, always first.** Look at the terms. If $$a_n$$ does not approach 0, the series diverges by the nth-term test, and you are done in one line. Try it on $$\textstyle \sum \tfrac{3n}{n+1}$$: the terms head to 3, not 0, so the series diverges and no further analysis is needed. If the terms do go to 0, the test says nothing at all ([the harmonic series](/2026/07/23/harmonic-series-surprises-of-infinity.html) is the standing counterexample), and you move on. This test can never prove convergence.

**Step 1: check for the two special families.** These have complete, exact theories, so look for them before doing any real work.

- Geometric, $$\textstyle \sum a r^n$$: converges exactly when $$\vert r\vert < 1$$, and you even get the sum, $$\tfrac{\text{first term}}{1 - r}$$. The field mark: the index appears only as an exponent.
- p-series, $$\textstyle \sum \tfrac{1}{n^p}$$: converges exactly when $$p > 1$$. The field mark: the index appears only as a base. Remember that $$p = 1$$ is the harmonic series, which diverges.
